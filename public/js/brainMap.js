...
function pointinpolygon(point, polygon) {
    ...
    var vs = polygon.map(function(ltlng) {
...
/**
 * brainMap creator
 * @param  {string} name    name of the map (must match the name of the folder)
 * @param  {L.map} map     map base element (from leaflet)
 * @param  {L.control.sidebar} sidebar sidebar element (from leaflet-sidebar)
 * @return {brainMap}          the brainMap object created
 */
 ...
 */
    ...
    /**
     * basePath is used to set the basePath of the server
     * (I think is not used now)
     * @type {String}
     */
    this.basePath = "";
    ...
    /**
     * options of the brainMap object
     * @type {Object}
     */
    this.state = {
        configError: false,
    ...
    /**
     * other options maybe better to include them in this.state
     * @type {Boolean}
     */
    ...
    this.inintialiseDrawControls = function() {
        ...
        this.map.on('draw:editstart', function() {
            self.state.unClickable = true; // disable clikkable regions on edit
            if (self.freeDraw) {
                self.freeDraw.setMode(L.FreeDraw.MODES.VIEW);
            }
        });
    ...
    this.defineLayer = function() {
        if (this.tilesUrlTemplate) {
            this.layer = L.tileLayer(this.tilesUrlTemplate, {
  ...
  */
    ...
    this.computeDensity = function(polygon) {
        ...
        if (window.Worker && this.useWorkers) {
            ...
            for (var tt = 0; tt < references.length; tt++) {
                ...
                } //compute area and holes by holes mask
                // read points
    ...
    this.addPolygon = function(polygon, notcompute) {
        ...
        polygon.bindContextMenu({
            ...
            contextmenuItems: [{
                ...
                iconCls: "fa fa-cog",
                callback: function() {
                    self.computeDensity(polygon)
                }
            }, '-', {
                text: "Results csv",
                iconCls: 'fa fa-file-text-o',
                callback: function() {
                    self.downloadRegionsCSV([polygon]);
                }
            }, {
                text: 'JSON',
                iconCls: 'fa fa-download',
                callback: function() {
                    self.downloadRegionsJSON(polygon);
                }
            }]
        });
    ...
    this.addHolesLayer = function() {
        var holesBounds = L.latLngBounds([
            [0, 0],
            [-255, 255]
        ]);
        this.holesLayer = L.tileLayer(this.holesMapUrlTemplate, {
            continuousWorld: true,
            maxZoom: 5,
            opacity: 0.5
        });
    ...
    this.clearPolygons = function() {
        var self = this;
        if (this.polygons) {
            this.polygons.map(function(pol) {
                if (pol) {
                    self.deletePolygon(pol)
                }
            });
        }
        ...
        addHeadPolInfoEl("id", ["density"],
            "fa fa-file", [{
                ...
                att: "id",
                vAt: "down-all-region-button"
            }, {
                att: "onclick",
                vAt: "brainmap.downloadRegionsCSV();"
            }]);
    }
    ...
    this.addControls = function() {
        ...
        this.controls = L.control.layers(baseMaps, overlayMaps, {
            position: "bottomleft",
            autoZIndex: false
        });
    ...
    this.addMarkersOf = function(e) {
        ...
        readPoints(null, ref[0], function(useless) {}, null, self, function(latlng, self, numberPoint) {
