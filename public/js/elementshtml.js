...
function toggleGrid(obj) {
    ...
    if (obj.state.showGrid) {
        var grid = L.simpleGraticule({
            interval: 256 / (obj.dim_px / 1024),
            redraw: 'moveend',
        })
        grid.addTo(obj.map);
        obj.grid = grid;
...
function addPolInfoEl(name, values, icon, iconAt) {
    ...
    */
    ...
    if (icon) {
        ...
        if (iconAt) {
            iconAt.map(function(x) {
                sp.setAttribute(x.att, x.vAt)
            });
...
function newModal(id, header, body, dialogSize) {
    if (document) {
